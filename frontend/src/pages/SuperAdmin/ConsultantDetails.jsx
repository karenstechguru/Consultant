...
const ConsultantDetails = () => {
    ...
    const Accept = () => {
        axios.patch("http://localhost:5137/conVerfication", { email })
            .then((res) => {
        ...
        nav("/consultantrequests");
    }

    const Decline = () => {
        axios.delete(`http://localhost:5137/conDelete/${email}`)
            .then((res) => {
                toast.success(res.data, {
                    position: "top-right",
                    autoClose: 3000,
    ...
    return (
        <div className="w-[50%] mx-auto mt-8 h-[70vh]">
            ...
            <div className="relative flex justify-center">
                <img src={`./public/consultantdetails/${photo}`} className="absolute right-0 top-5 border-solid w-52 h-52" alt={name} />
            </div>
