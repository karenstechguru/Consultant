...
const BookSchedule = () => {
    let k = useLocation();
    let { email,consultantName,expertise } = k.state;
    ...
    const getData = () => {
        if (email != null) {
            axios.get(`http://localhost:5137/getSchedule/${email}`)
                .then((res) => {
    ...
    const userBooking = () => {
        if (date !== "" && time !== "") {
            let uid= uuidv4();
            axios.post(`http://localhost:5137/Booking/addBooking`, { name, email: UserEmail, consultantEmail: email, date, time,meetingId:uid,consultantName,expertise,sta:"Upcomming",reason:"N/A" })
                .then((res) => {
                    console.log(res.data)
                })
                .catch((e) => console.log(e))

            axios.patch('http://localhost:5137/scheduleStatus', { email, date, time })
                .then((res) => {
                    console.log(res.data)
                })
                .catch((e) => console.log(e))
