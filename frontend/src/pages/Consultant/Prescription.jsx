...
const Prescription = () => {
    ...
    const consultantEmail = Cookies.get('email');
    const consultantName = Cookies.get('name');
    ...
    const handleSubmit = (e) => {
        ...
        if (name.trim() !== '' && email.trim() !== '' && prescription.length > 0 && prescription.every(item => item.medicine.trim() !== '' && item.meal.trim() !== '' && item.times.length > 0 && item.dosage.trim() !== '' && item.days.trim() !== '')) {
            ...
            console.log(time);
            console.log(advice);
            axios.post("http://localhost:5137/pres/addPrescription",{name,email,consultantName,consultantEmail,date,time,prescription,advice})
            .then((res)=>{
                // alert(res.data)
